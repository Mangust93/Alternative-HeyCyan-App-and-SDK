...
/**
 * Real, file-backed diagnostic log for manual phone testing.
 *
 * Lives in the optional, standalone :debug-log-tools module. It has NO compile
 * dependency on :app and never touches the real glasses / media / BLE / P2P flow or
 * the Moonshine runtime. It only writes the events callers hand it to app-specific
 * storage and reads them back.
 *
 * Events are stored, one per line, in:
 *   filesDir/debug-log-tools/debug-events.log
 * with the shape:
...
object DebugLogStore {
    ...
    private const val DIR_NAME = "debug-log-tools"
    private const val FILE_NAME = "debug-events.log"
    ...
    /**
     * Append one real diagnostic event. Safe to call from any thread.
     *
     * @param tag short subsystem tag (e.g. "BLE", "MEDIA", "AUTOMATION").
    ...
    fun append(context: Context, tag: String, message: String, throwable: Throwable? = null) {
        val level = if (throwable != null) "E" else "I"
    ...
    fun buildDiagnosticBundle(context: Context): String {
        ...
        return buildString {
            appendLine("=== CyanBridge diagnostic bundle ===")
